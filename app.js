...
app.get("/players/:playerId/playerScores", async (request, response) => {
  const { playerId } = request.params;
  const statistics = `select player_id,player_name,
    sum(score),sum(fours),sum(sixes) from player_match_score natural join player_details where player_id=${playerId};`;
  const result = await database.get(statistics);
  response.send({
